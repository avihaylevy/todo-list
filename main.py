...
from schemas import AllowedOrderBy, ResponseMessage, TodoDetail, TodoItem, TodoResponse, TodoUpdate
import logic
...
def get_tasks(status: str = None, after_id: int = 0, limit: int = 10,
              db: Session = Depends(get_db), order_by: AllowedOrderBy = AllowedOrderBy.created_at,
              priority: str = None, due_date: datetime | None = None, due_soon: bool = False):
    return logic.view_task(db, status=status, after_id=after_id, limit=limit,
                           order_by=order_by, priority=priority, due_date=due_date, due_soon=due_soon)
